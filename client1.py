...
def run_client(host='127.0.0.1', port=5000):
    ...
    print("Opcode 10 - KEY VERIFICATION\n"
          "[Client1] Sending KEY VERIFICATION with Public Key of Client to Server.")
    
    s.sendall(struct.pack('!I', 10) + struct.pack('!I', client_pub_key))

    resp = s.recv(8)
    if len(resp) < 8:
        print("[Client1] Failed to complete handshake.")
        s.close()
        return
    ...
    if srv_opcode != 10:
        print(f"[Client1] Unexpected opcode {srv_opcode}, Expected Opcode = 10 Closing")
        s.close()
        return
    print("Opcode 10 - KEY VERIFICATION\n"
          f"[Client1] Server replied with KEY VERIFICATION with Public Key of Server = {srv_pub_key}.\n"
          "Key verification done.")

    shared_secret = compute_dh_shared_key(srv_pub_key, x, DH_PRIME)
    K1, K2 = derive_des_keys(shared_secret)
    print(f"[Client1] Derived K1={K1.hex()}, K2={K2.hex()}")

    while True:

        hdr = s.recv(4)
        if not hdr:
            print("[Client1] Server closed connection.")
            break
        opcode_20 = struct.unpack('!I', hdr)[0]

        if opcode_20 == 50:

            print("50 → DISCONNECT\n[Client1] Server ended session.")
            break

        if opcode_20 == 20:
            print("Opcode 20 - SESSION TOKEN\n"
                  "[Client1] Received SESSION TOKEN with Encryption. Decrypting with K1.")
        else:
            print(f"[Client1] Unexpected opcode {opcode_20}, expected 20 Closing")
            break

        enc_token = s.recv(16)
        if len(enc_token) < 16:
            print("[Client1] Incomplete token data.")
            break

        session_token = des_decrypt(K1, enc_token)
        print(f"[Client1] Session token = {session_token.hex()}")

        user_input = input("[Client1] Enter numeric data (or 'quit' to disconnect): ").strip()
        if user_input.lower() == 'quit':

            print("Opcode 50 - DISCONNECT\n[Client1] Sending (50) to server.")
        ...
        print("Opcode 30 - CLIENT ENC DATA\n"
              "[Client1] Sending Double DES Encrypted Data to server.")
        ...
        hdr = s.recv(4)
        if len(hdr) < 4:
            print("[Client1] Server closed unexpectedly.")
            break
        opcode_40 = struct.unpack('!I', hdr)[0]

        if opcode_40 != 40:
            print(f"[Client1] Unexpected opcode {opcode_40}, expected 40. Closing.")
            break
        print("Opcode 40 - ENC AGGR RESULT\n"
              "[Client1] Received Aggregated Data from Server Decrypting the data.")

        len_buf = s.recv(4)
        if len(len_buf) < 4:
            print("[Client1] Incomplete aggregator length.")
            break
        ...
        hmac_agg = s.recv(32)  
        if len(hmac_agg) < 32:
            print("[Client1] Incomplete aggregator HMAC.")
            break
        ...
        try:
            ...
            print("[Client1] Aggregator HMAC verification FAILED.")
            continue

        try:
            agg_decrypted = double_des_decrypt(enc_agg, K1, K2).decode('utf-8')
            print(f"[Client1] The server's aggregated result = {agg_decrypted}\n")
        except Exception as e:
            print(f"[Client1] Decryption error: {e}\n")
